-- Remove Pending Status Completely from Entire App (FIXED)
-- This script ensures all orders are automatically confirmed when placed
-- No more "Pending" status anywhere in the system
-- FIXED: Removed references to non-existent order_status column

-- ========================================
-- 1. UPDATE ALL EXISTING PENDING ORDERS
-- ========================================

-- Update all existing pending orders to confirmed (using only status column)
UPDATE orders 
SET 
    status = 'confirmed',
    updated_at = NOW()
...
BEGIN
    ...
    LOOP
        ...
        -- Create order with 'confirmed' status (NO PENDING) - using only status column
        INSERT INTO orders (
            user_id,
            quotation_code,
            merchant_code,
...
-- Function to automatically confirm all orders (using only status column)
CREATE OR REPLACE FUNCTION auto_confirm_all_orders()
RETURNS TRIGGER AS $$
BEGIN
    -- Force all orders to be confirmed (only set status column)
