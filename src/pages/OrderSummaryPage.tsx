...
const OrderSummaryPage = () => {
        ...
        const fetchCartProducts = async () => {
            ...
            if (!error && data) {
                ...
                const merged = cart.map(item => {
                    ...
                    if (product) {
                        // If this is a quotation-based item, keep the quotation price
                        if (item.quotation_id) {
                            return { 
                                ...product, 
                                quantity: item.quantity,
                                price: item.price, // Keep quotation price
                                quotation_price: item.price, // Store as quotation_price for clarity
    ...
    const subtotal = cartProducts.reduce((sum, item) => {
        // For quotation-based items, use the quotation price
        if (item.quotation_id && item.quotation_price) {
            return sum + (Number(item.quotation_price) * item.quantity);
        }
    ...
    const handleMockOrderPlacement = async () => {
        ...
        // If this order is from a quotation, update the quotation status to completed
        if (quotationId) {
            ...
            if (quotationError) {
                ...
                console.log('Quotation status updated to completed');
            }
        }
                                                        ...
                                                        <div className="flex flex-col">
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <span className="font-bold text-base sm:text-lg text-green-700">
                                                                    ₹{item.quotation_id && item.quotation_price ? item.quotation_price : item.price}
                                                                </span>
                                                                {item.quotation_id && (
