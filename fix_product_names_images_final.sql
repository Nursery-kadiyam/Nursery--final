-- COMPREHENSIVE FIX FOR PRODUCT NAMES AND IMAGES - FINAL VERSION
...
-- 3. Drop existing functions if they exist (to avoid return type conflicts)
...
-- 4. Create merchant orders function with proper product joins
...
-- 5. Create merchant order details function
...
-- 6. Create user orders function (fixed return type)
...
-- 8. Populate missing order_items for ORD-2025-0005 with proper product linking
...
-- 9. Update existing order_items with correct unit_price and subtotal
UPDATE order_items 
SET unit_price = CASE 
...
-- 11. Verify specific order ORD-2025-0005
SELECT 
...
-- 12. Check for any missing product links
SELECT 
...
-- 13. Final verification - check function signatures
SELECT 
...
FROM pg_proc 
WHERE proname IN ('get_merchant_orders_privacy_protected', 'get_merchant_order_details', 'get_user_orders_with_items');